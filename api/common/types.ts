...
export const FullConfig = Type.Object({
    ...
    'media::playback::url': Type.String({ description: 'Optional browser-facing base URL for HLS playback (e.g. a standard-port reverse proxy) when it differs from media::url. Falls back to media::url when unset.' }),
    'media::ingest::internal::host': Type.String({ description: 'Optional internal docker-network hostname to rewrite RTSP proxy sources to when their hostname matches media::url\'s own hostname. Deployments where the media service pulling its own public hostname as a proxy source would hairpin back out through NAT can set this to an internal alias that resolves directly. Leave unset to disable rewriting (default).' }),
