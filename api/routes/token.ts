...
export default async function router(schema: any, config: Config) {
    ...
    await schema.post('/token', {
        ...
        try {
            ...
            if (req.auth instanceof AuthResource) throw new Err(400, null, 'Tokens can only be generated by an JWT authenticated user');

            const token = await config.models.Token.generate({
                ...req.body,
                token: 'etl.' + jwt.sign({ id: req.auth.email, access: 'profile' }, config.SigningSecret),
    ...
    await schema.patch('/token/:id', {
        ...
        try {
            ...
            if (req.auth instanceof AuthResource) throw new Err(400, null, 'Tokens can only be updated by an JWT authenticated user');
    ...
    await schema.delete('/token/:id', {
        ...
        try {
            ...
            if (req.auth instanceof AuthResource) throw new Err(400, null, 'Tokens can only be deleted by an JWT authenticated user');
